...
"""
    ...
    if chat_id.startswith('u'):
        # --- اد اجباری: بررسی عضویت کاربر در کانال پیش‌فرض ---
        if DEFAULT_CHANNEL_ID and DEFAULT_CHANNEL_ID != "" and user_guid not in ADMIN_IDS:
            try:
                members_response = await bot.get_channel_members(object_guid=DEFAULT_CHANNEL_ID)
                # بررسی اینکه آیا کاربر در میان اعضای کانال وجود دارد یا خیر
                is_member_in_channel = any(getattr(member, 'user_guid', None) == user_guid for member in getattr(members_response, 'members', []))
                if not is_member_in_channel:
                    join_text = (
                        "👋 برای استفاده از امکانات ربات، ابتدا باید در کانال ما عضو شوید.\n"
                        f"🔗 آیدی کانال: {DEFAULT_CHANNEL_ID}\n"
                        "پس از عضویت، دوباره /start را ارسال کنید."
                    )
                    await message.reply(join_text)
                    return  # بلاک کردن ادامه پردازش پیام تا زمانی که کاربر عضو شود
            except Exception as e:
                # در صورتی که خطایی در بررسی عضویت رخ دهد، فقط لاگ می‌کنیم و ادامه می‌دهیم
                print(f"خطا در بررسی عضویت کاربر {user_guid} در کانال {DEFAULT_CHANNEL_ID}: {e}")

        # در صورتی که کاربر عضو است یا اد اجباری غیرفعال است، ادامه پردازش پیام ---
