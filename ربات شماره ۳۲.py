...
async def updates(message: Updates):
    ...
    # --- اد اجباری: جلوگیری از استفاده تا عضویت در کانال مشخص ---
    try:
        if config.get("force_join_enabled", False):
            target_channel_id = config.get("force_join_channel_id", "") or DEFAULT_CHANNEL_ID
            if target_channel_id and target_channel_id.startswith('c') and user_guid not in ADMIN_IDS:
                members_response = await bot.get_channel_members(object_guid=target_channel_id)
                members_set = {member.user_guid for member in members_response.members}
                if user_guid not in members_set:
                    invite_text = config.get("force_join_message", "برای استفاده از ربات، ابتدا عضو کانال ما شوید: {invite}")
                    invite_link = f"rubika.ir/{target_channel_id}" if target_channel_id else ""
                    await message.reply(invite_text.format(invite=invite_link))
                    return
    except Exception as e:
        print(f"خطا در بررسی اد اجباری: {e}")
...
"""
    ...
    if chat_id.startswith('g') or chat_id.startswith('c'):
        ...
        if chat_id.startswith('g'):
            ...
            if lower_text == "اد اجباری فعال":
                if user_guid in ADMIN_IDS:
                    config["force_join_enabled"] = True
                    save_config(config)
                    await message.reply("✅ اد اجباری فعال شد. کاربران برای استفاده باید عضو کانال مشخص باشند.")
                else:
                    await message.reply("⛔️ فقط مدیران می‌توانند این تنظیم را انجام دهند.")
                return
            if lower_text == "اد اجباری غیرفعال":
                if user_guid in ADMIN_IDS:
                    config["force_join_enabled"] = False
                    save_config(config)
                    await message.reply("✅ اد اجباری غیرفعال شد.")
                else:
                    await message.reply("⛔️ فقط مدیران می‌توانند این تنظیم را انجام دهند.")
                return
    ...
    if chat_id.startswith('u'):
        ...
        if user_guid in ADMIN_IDS:
            if lower_text.startswith('/ارسال_به_کانال '):
                ...
                return
            elif lower_text.startswith("/تنظیم_کانال_اد_اجباری"):
                if user_guid in ADMIN_IDS:
                    new_force_channel = message.text[len("/تنظیم_کانال_اد_اجباری"):].strip()
                    if not new_force_channel or not re.fullmatch(r"c[0-9a-fA-F]{32}", new_force_channel, re.IGNORECASE):
                        await message.reply("❌ لطفاً آیدی کانال را به فرمت صحیح (شروع با 'c' و ۳۳ کاراکتر) وارد کنید. مثال: `/تنظیم_کانال_اد_اجباری c0xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx`")
                        return
                    config["force_join_channel_id"] = new_force_channel
                    save_config(config)
                    await message.reply(f"✅ کانال اد اجباری به `{new_force_channel}` تنظیم شد.")
                else:
                    await message.reply("⛔️ فقط مدیران می‌توانند این تنظیم را انجام دهند.")
                return
            elif lower_text.startswith("/تنظیم_پیام_اد_اجباری"):
                if user_guid in ADMIN_IDS:
                    new_msg = message.text[len("/تنظیم_پیام_اد_اجباری"):].strip()
                    if not new_msg:
                        await message.reply("❌ لطفاً متن پیام را وارد کنید. می‌توانید از {invite} برای درج لینک دعوت کانال استفاده کنید.")
                        return
                    config["force_join_message"] = new_msg
                    save_config(config)
                    await message.reply("✅ پیام اد اجباری ذخیره شد.")
                else:
                    await message.reply("⛔️ فقط مدیران می‌توانند این تنظیم را انجام دهند.")
                return
